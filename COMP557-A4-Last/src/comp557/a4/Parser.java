...
public class Parser {
	...
	public static Render createRender(Node dataNode) {
		...
		Node jitteringAttr = dataNode.getAttributes().getNamedItem("jitter");
		if(jitteringAttr != null){
			Scanner s = new Scanner(jitteringAttr.getNodeValue());
			String val = s.next();
			if(val.equals("true")) {
				render.jittering = true;
			}else{
				render.jittering = false;
			}
			s.close();
		}else{
			render.jittering = false;
		}
		Node shadowAttr = dataNode.getAttributes().getNamedItem("smoothShadow");
		if(shadowAttr != null){
			Scanner s = new Scanner(shadowAttr.getNodeValue());
			String val = s.next();
			if(val.equals("true")) {
				render.smoothShadows = true;
			}else{
				render.smoothShadows = false;
			}
			s.close();;
		}else{
			render.smoothShadows = false;
		}
		Node lightSizeAttr = dataNode.getAttributes().getNamedItem("lightSize");
		if(lightSizeAttr != null){
			Scanner s = new Scanner(lightSizeAttr.getNodeValue());
			render.lightSize = s.nextDouble();
			s.close();
		}else{
			render.lightSize = 1;
		}
		Node reflectionAttr = dataNode.getAttributes().getNamedItem("reflectionLevels");
		if(reflectionAttr != null){
			Scanner s = new Scanner(reflectionAttr.getNodeValue());
			render.reflectionLevels = s.nextInt();
			s.close();
		}else{
			render.reflectionLevels = 0;
		}
    	NodeList nodeList = dataNode.getChildNodes();
    	for (int i = 0; i < nodeList.getLength(); i++) {
    		Node n = nodeList.item(i);
